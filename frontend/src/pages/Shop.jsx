    ...
    <div data-testid="shop-page" className="pt-32 pb-24">
      <section className="max-w-7xl mx-auto px-6 sm:px-12">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-end mb-20">
          ...
          <div className="md:col-span-4 md:col-start-9">
            <p className="text-[#5C5852] leading-relaxed">
              A small selection of pieces I&apos;ve built. Some are for sale,
              some have already found a home, and some are just here for the
              record. If something catches your eye, get in touch.
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          >
            {products.map((p, idx) => {
              const status = STATUS_META[p.status] || STATUS_META.not_for_sale;
              return (
                <Link
                  key={p.id}
                  to={`/portfolio/${p.id}`}
